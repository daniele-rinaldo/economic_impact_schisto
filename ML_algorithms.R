...
############### MC Algorithms: Rlasso, Tree, Neuralnet, Nnet, Boosting, Random Forest ################### 
...
boost <- function(datause, dataout, form_x, form_y, distribution, option){
  
  form           <- as.formula(paste(form_y, "~", form_x));
  # boostfit       <- do.call(gbm, append(list(formula=form, distribution=distribution, data=datause), option))
  boostfit       <- gbm(formula=form, distribution=distribution, data=datause,cv.folds=option$cv.folds, n.trees=option$n.trees,interaction.depth=option$interaction.depth,bag.fraction = option$bag.fraction,shrinkage=option$shrinkage)
  
  if(option[['cv.folds']]>1)  {best           <- gbm.perf(boostfit,plot.it=FALSE,method="cv")}
...
RF <- function(datause, dataout,  form_x, form_y, x=NA, y=NA, xout=NA, yout=NA, nodesize, arg, reg=TRUE, tune=FALSE){
  ...
  if(is.na(x)){
    ...
    if(tune==TRUE){
      fit.p           <- lm(form,  x = TRUE, y = TRUE, data=datause); 
      forest_t        <- tuneRF(x=fit.p$x, y=fit.p$y, mtryStart=floor(sqrt(ncol(fit.p$x))), stepFactor=1.5, improve=0.05, nodesize=5, ntree=as.numeric(arguments$Forest$ntree), doBest=TRUE, plot=FALSE, trace=FALSE)
      min             <- forest_t$mtry
